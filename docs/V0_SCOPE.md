...
- [x] Every AI mutation appears in `ai_actions` with input, output and outcome — `Arguments` is the input and `Outcome` is the outcome, both asserted by `ToolPipelineTests`; `Result` is the output, a jsonb object carrying the note, the question and the answer a call reported. Asserted on `search_memory` in `MemoryToolPipelineTests`, which is the case that needed it — a tool that creates nothing has a null `entity_id`, so before this the row could not say what the couple was told. STATUS debt 48, paid
