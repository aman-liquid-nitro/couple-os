...
| Tests | 324 plus 42 SQL assertions, all shown capable of failing (11 need a model provider configured; 4 of those fail *with* one, on expectations the eval set has not caught up with — debts 21 and 35) |
...
| Eval cases running | 8 of 55 — 4 passing, 4 failing on their own expectations rather than on behaviour |
...
| Eval harness exists and runs, 3 cases wired in | done — 8 run since the task tools landed; `EvalCoverage` reports the 24 still blocked |
