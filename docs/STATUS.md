...
**Updated:** 2026-08-14 · **Milestone:** M5 met. **Every milestone's exit criteria are met and machine-checked.** V0_SCOPE's own done-checklist is **fourteen of fourteen** — debt 47's memory corpus was built and paid, and debt 48 (no tool output in `ai_actions`) is now paid too. Building the corpus found two ranking defects, debts 50 and 51, which is what a corpus is for. **Done, in the sense the checklist defines.** Not yet validated either way: that is the week of real use, and the stack is now ready to run it — self-hosted behind Tailscale, per [DEPLOY.md](./DEPLOY.md), which needed no application code.
...
| | |
|---|---|
| Milestone | M5 (attachments and the read surface) closed; M0–M4 closed. **All five milestones met**; V0_SCOPE's checklist **14/14** |
| Commits | 58 |
| Architecture decisions | 14 |
| Tests | 516 plus 42 SQL assertions, all shown capable of failing (54 need a model provider configured) |
...
| Deployment | Settled: self-hosted behind Tailscale. `docker-compose.live.yml` + [DEPLOY.md](./DEPLOY.md). Magic links stay on maildev (debt 53), inference on hosted `gemma4:31b` |
...
| A ranking that measures relevance rather than brevity, and a trigram fallback that fires (debts 50, 51) | V1 |
| A way out of the private thread — `share_memory` was described in three documents and built in none (debt 49) | V1 |
...
14. ~~**Base images float on the `10.0` tag, not a digest.**~~ The build is reproducible in the sense that matters today — restore is pinned by `Directory.Packages.props` — but two builds a month apart can sit on different SDK patches. Pin when there is somewhere to deploy to. **Paid, now that there is.** All four are pinned by digest: `sdk:10.0` and `aspnet:10.0` in the Dockerfile, `pgvector/pgvector:pg16` and `maildev/maildev:2.1.0` in compose. The tags are kept beside the digests for readability and Docker ignores them, so the two cannot disagree in a way that changes a build. The Postgres one turns out to matter most and was the least obvious: `pg16` is a moving tag, and a minor-version change under an existing data directory is the one upgrade Postgres will not perform silently — it refuses to start, mid-week, on a stack that was working. The cost is that security patches now arrive when someone re-pins. *(Paid)*
...
17. ~~**`PublicBaseUrl` is unset, so link URLs come from the request's `Host` header.**~~ Correct for localhost and containers, and attacker-controlled in general: a forged Host would mint links pointing elsewhere. Set it before this is reachable from a network you do not control. **Paid.** `docker-compose.live.yml` requires it and refuses to start without it, which is the right shape for this one: an absent value is not a visible failure but a security property quietly switched off, and Compose's `${VAR:?message}` turns that into a startup error naming the variable.
...
23. ~~**No CI.**~~ **Settled as a deliberate no, and the entry stays because the thing it worried about is still true.** M4 built a hosted runner and it was removed the same milestone. The reasoning is in IMPLEMENTATION_PLAN.md's first decision: for one person on one machine, a runner buys "it happens when you forget" and "it happens on a clean machine", and charges a hosted model call on every push to every branch for a suite whose own design calls a run a sample. So the gate is `scripts/check.sh`, run on demand, and it is the same gate — same steps, same order, same `tools/CoupleOS.EvalGate` deciding. **The clean-machine half cost something within a day of being given up, and this is what it looked like.** `.gitattributes` says `* text=auto`, which stores LF and hands Windows CRLF *on checkout*. `FileRewriter` builds its output with `"\n"`, and its tests compare that against raw string literals — whose line endings are whatever the checkout produced. So fourteen tests fail on any fresh Windows clone and pass for anyone whose working copy predates the checkout that converted them, which was everyone who had ever run the suite. The run recorded as green above was green, on a working tree that had never been re-materialised. Found by switching branches, which is a checkout, which is all it takes. Fixed at the repository rather than in the assertions — `*.cs text eol=lf`, so the working tree matches what the repository already stores — because normalising inside every comparison makes each test carry a workaround for a setting. One file, `MemoryToolPipelineTests.cs`, turned out to have been committed with CRLF bytes and is renormalised in the same change. Nothing about this needed a model, a database or a container: a `dotnet test` on a clean clone would have caught it on the first push, and that is the specific thing no longer being done.
...
47. ~~**`search_memory` has never been asked a question with more than one plausible answer.**~~ V0_SCOPE's done-checklist wants relevant results "over a seeded corpus of ≥100 memories" and there is no corpus — every search test writes one or two rows carrying a token nothing else contains, so the assertion is that the query matches at all. Ranking is the part that was supposed to be measured: `MemorySearch` blends lexical score with recency, and a corpus of two cannot tell a good blend from a bad one, nor from no blend. This is not the same gap as the missing vector index (ADR 0002 defers that until ~10k rows, deliberately) — a hundred rows is small enough to stay lexical and large enough for a wrong ordering to show. It wants a fixture rather than a design: a hundred memories across the types SPEC.md §8 names, and a handful of queries whose *first* result is asserted rather than whose result set is non-empty. Cheap to build and the reason it was not built is that nothing was blocked on it — which is exactly the shape of a box ticked by assumption, so the box is now unticked. *(V1, and small)* **Paid, and it did not come back empty-handed.** `MemoryCorpusFixture` seeds 104 memories for a couple of its own — clustered on purpose, because a ranking is only wrong in the presence of neighbours, and a hundred unrelated sentences would have been a hundred copies of the tests that already existed. Ages are relative to `now()` rather than fixed dates, so a recency assertion made today still means the same thing next March (debt 21's mistake, avoided rather than repeated). `MemorySearchRankingTests` asserts what comes back *first*: the ordinary case where every term of the blend agrees, the recency term in isolation, a type filter promoting a row from third place rather than shortening a list, and `LIMIT` applying after `ORDER BY` instead of before it. **Two of the seven tests assert behaviour that is wrong** — debts 50 and 51, both found by the corpus within minutes of it existing, and neither visible on two rows. One process note worth keeping: the recency test passed in its first form *with the recency term deleted from the query*, because the `ORDER BY` ends in `created_at DESC` and two rows differing only in age come out newest-first from the tiebreak alone. It was measuring the tiebreak and reporting it as the ranking — debt 38's shape exactly, caught this time by mutating the source and watching which tests failed. The fixture now gives the older row double the importance, so the newer one has to win against something. *(Paid after M5, in the reconciliation that unticked it)*

48. ~~**An `ai_actions` row records what was asked and how it ended, but not what came back.**~~ The `result` jsonb column exists in `data/schema.sql` and has no property on `AiAction`, so nothing writes it and nothing could read it. `Arguments` is the input and `Outcome` is the outcome; `entity_type`/`entity_id` point at the row that was created, which is the closest thing present and is still a pointer rather than a record — it says *which* shopping item, not what the tool reported about it, and for a tool that creates nothing (`search_memory`, `request_clarification`) it is null. So V0_SCOPE's "input, output and outcome" is two of three, and the audit trail SPEC.md §50 wants cannot answer "what did the system tell the couple" from storage; only the rendered report says that, and the report is not kept per call. The narrow fix is one mapped property and one line in the dispatcher, and the reason to think before taking it is that a tool result can carry the couple's own text back into a second place — an audit table is exactly where a private value should not appear twice by accident. **Paid, and the thinking came out in favour.** `AiAction.Result` carries the three things a call can report beyond its row — the note, the question, the answer — serialised in `AuditingToolDispatcher` and null when there are none, because an empty object would read as "the tool returned nothing", which is a claim. The privacy question has an answer already in the schema: `couple_scope` on `ai_actions` is `couple_id = app_current_couple() AND user_id = app_current_user()`, so a row is readable only by the person whose call it was, and a private answer cannot reach the partner through it. That is stricter than the couple-wide policy most tables carry, and it is why this column is safe where the same text in `audit_logs` needs `visibility` and `owner_user_id` to be safe. Asserted where it matters — on `search_memory`, the tool with no `entity_id` to fall back on. *(Paid)*

49. **`share_memory` does not exist, and three documents say it does.** V0_SCOPE listed it in scope as the private thread's one exit; ADR 0009 says there is no way to write a shared record from a private thread "except by calling `share_memory`"; this file called it the only path across the direction that must never open. TOOLS.md tiers it V1, and V1 is where it stayed. The single occurrence in the source is a doc comment on `Memory.cs` naming it as one of the ways a memory's status changes — no tool, no registration, no test. **Nothing was blocked on it**, which is why five milestones went by without anyone noticing: every registered tool writes to the surface it was called from, so no code path ever reached for it. The privacy consequence is the harmless direction — the door is not confirmed-but-open, it is absent, and private-to-shared is the crossing the whole design exists to prevent. What is actually lost is the **exit**: the private thread is where a plan gets worked out (ADR 0009's own framing — thinking partner, not filing clerk), and the only way that plan reaches the couple is the person retyping it into `shared.md`. That is a product gap wearing a privacy tool's name, and it should be sized as one: it needs a confirmation surface in the thread, not just a tool contract. Documents corrected in place rather than quietly, because a doc that describes an absent safety mechanism is worse than one that omits it. *(V1)*

50. **The ranking rewards brevity and calls it relevance.** Two memories in the corpus contain "insurance" once each, at the same age and the same importance. One reads *"The car insurance renewal is due in March and we agreed to switch providers because the current one raised the premium after the claim"*. The other reads *"Insurance sorted"*. The empty one wins, and not narrowly. The cause is arithmetic rather than judgement: `similarity(content, query)` compares the query's trigrams against the **whole** content, so a longer memory scores lower for being longer — 0.09 against 0.50 for this pair — and that term is *doubled* in the blend while `ts_rank`, the only term measuring relevance, is identical for both single-occurrence rows (0.0991 against 0.0974 — a difference of 0.007 after its ×4). Measured, not reasoned: `ts_rank` with the default normalisation does not account for document length at all, so the length penalty enters only through the trigram term, where it is an artifact. The consequence is not cosmetic, because the model is handed the top rows and answers from them: the couple asks about the insurance and is told it is sorted, by a memory containing no fact. The fix is `word_similarity(query, content)`, which compares the query against the best-matching *extent* of the content and returns 0.583 for all three lengths of the detergent sentence — verified against the running database. That changes every ranking in the system, so it wants the corpus it now has and a before-and-after, not a one-line edit. Characterised by `A_short_empty_memory_outranks_a_long_precise_one_on_the_same_word`, which is named for what happens and fails when this is fixed. *(V1)*

51. **The trigram branch never fires, so a misspelling finds nothing.** `MemorySearch`'s own comment says full text catches the phrase and trigram "catches the misspelling and the partial word, and uses the GIN index the schema already builds for it". It does not, for anything a person would write. `content % 'detrgent'` tests whole-string similarity against `pg_trgm.similarity_threshold`, which is 0.3: the bare word *detergent* scores 0.58 and matches, **"We are out of detergent" scores 0.28 and does not**, and every longer memory scores lower still. So the OR-branch only fires when the content is barely more than the search term — that is, when full text would almost certainly have matched anyway. It is not dead code, it is worse: it is a fallback that is present, indexed, documented, and inert, and the corpus is the first thing to ask it a question it should have answered. Same root cause as debt 50 and the same fix — `<%` / `word_similarity` compares against the best-matching extent and is length-independent — with the caveat that `word_similarity_threshold` defaults to 0.6 and the honest score for a one-letter typo is 0.583, so the threshold has to be chosen deliberately rather than inherited. Characterised by `A_misspelling_finds_nothing_once_the_memory_is_a_sentence`, which asserts the empty result and the correctly-spelled control beside it. *(V1)*

52. **The backups are two files on the same disk as the database they protect.** [DEPLOY.md](./DEPLOY.md) gives a `pg_dump` and a `tar` of the attachments volume, run by hand, written locally. Nothing schedules them, nothing encrypts them and nothing puts them anywhere else, so the whole arrangement survives `docker compose down -v` and a mistaken `DROP` — which are the two failures a week of real use actually presents — and survives no failure of the disk. ARCHITECTURE.md §7 asks for nightly, encrypted, off-site, and that is the right target for something people depend on; it is not the right amount of machinery to build before knowing whether anyone will use this. The restore path is tested rather than assumed, which is the half that is usually skipped and the half that fails: the dump carries the 22 row-level-security policies as well as the rows, verified by restoring into a scratch database and counting them. A restore that brought back the data without the policies would look completely successful and would have opened every private memory to the partner. *(V1, and the day it stops being a seven-day experiment)*

53. **The maildev inbox is the credential.** ADR 0007 makes a magic link the entire sign-in, and the validation run keeps maildev as the sender rather than a real SMTP provider — so the link is read from a web UI that shows every message it has ever received to anyone who opens it. On a two-person tailnet holding only the couple's own devices this is a considered trade for a week: no sender account, no credentials in `.env`, no deliverability to debug on day one. It stops being a considered trade the moment a third device joins the tailnet, a friend is shared in, or the week is extended — at which point either partner can sign in as the other, and nothing in the audit trail would look wrong, because from the application's side nothing *is* wrong. Recorded here rather than only in the runbook because the condition that makes it acceptable is about the tailnet rather than about the code, and nothing in the repository can assert it. *(Before any use beyond the validation week)*
